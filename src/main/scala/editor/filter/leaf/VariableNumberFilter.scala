 ...
 * @author Alec Roelke
 */
class VariableNumberFilter(t: CardAttribute, value: (Card) => Iterable[Double], variable: (Card) => Boolean) extends NumberFilter(t, value(_).map(java.lang.Double(_)).toSeq.asJava) {
  var varies = false
