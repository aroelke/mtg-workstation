...
public enum CardAttribute implements Supplier<FilterLeaf>, Comparator<Object>
{
    ...
    POWER(
        ...
        List.class,
        (a) -> new VariableNumberFilter(a, (f) -> f.power().value(), Card::powerVariable),
    ...
    TOUGHNESS(
        ...
        List.class,
        (a) -> new VariableNumberFilter(a, (f) -> f.toughness().value(), Card::toughnessVariable),
        (a, b) -> ((CombatStat)a).compareTo((CombatStat)b)
    ),
    ...
    LOYALTY(
        ...
        List.class,
        (a) -> new VariableNumberFilter(a, (f) -> f.loyalty().value(), Card::loyaltyVariable),
    ...
    CATEGORIES("Categories", null, Set.class, (a, b) -> {
        var first = CollectionUtils.convertToSet(a, Categorization.class).stream().sorted(Comparator.comparing(Categorization::name)).collect(Collectors.toList());
        var second = CollectionUtils.convertToSet(b, Categorization.class).stream().sorted(Comparator.comparing(Categorization::name)).collect(Collectors.toList());
        for (int i = 0; i < Math.min(first.size(), second.size()); i++)
        {
            int diff = first.get(i).name().compareTo(second.get(i).name());
            if (diff != 0)
                return diff;
        }
